...
// Funções para PIX
function contribuirPix(valor) {
    document.getElementById('pixValor').textContent = valor;
    document.getElementById('pixModal').style.display = 'flex';
    trackEvent('click', 'presente', 'pix_valor_' + valor);
}

function contribuirPixCustom() {
    const valor = document.getElementById('valorCustom').value;
    if (!valor || valor <= 0) {
    ...
    document.getElementById('pixValor').textContent = valor;
    document.getElementById('pixModal').style.display = 'flex';
    trackEvent('click', 'presente', 'pix_custom_' + valor);
}
...
function contribuirCartao(valor) {
    // Aqui você pode integrar com PagSeguro, Mercado Pago, etc.
    const url = gerarUrlPagamento(valor);
    window.open(url, '_blank');
    trackEvent('click', 'presente', 'cartao_valor_' + valor);
}

function contribuirCartaoCustom() {
    const valor = document.getElementById('valorCartaoCustom').value;
    if (!valor || valor <= 0) {
        alert('Por favor, insira um valor válido');
        return;
    }
    const url = gerarUrlPagamento(valor);
    window.open(url, '_blank');
    trackEvent('click', 'presente', 'cartao_custom_' + valor);
}

function gerarUrlPagamento(valor) {
    // Exemplo de integração com PagSeguro
    // Substitua pela sua URL real de pagamento
    const baseUrl = 'https://pagseguro.uol.com.br/checkout/checkout.jhtml';
    const params = new URLSearchParams({
        'email_cobranca': 'seuemail@exemplo.com',
        'type': 'CP',
        'currency': 'BRL',
        'item_id_1': '001',
        'item_description_1': 'Presente de Casamento Carolina & Thiago',
        'item_amount_1': valor.toString().replace('.', ','),
        'item_quantity_1': '1',
        'reference_prefix': 'CASAMENTO',
        'reference': 'PRESENTE_' + Date.now()
    });
